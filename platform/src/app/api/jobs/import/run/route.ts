...
import {
  ...
  sheetToRows,
  type JobImportTabResult,
} from "@/lib/job-sheet-import";
import { MODERN_JOB_COLUMN_KEYS } from "@/lib/sheet-job-columns";

/** Large workbooks + many upserts; raise on hosts that honor this (e.g. Vercel). */
export const maxDuration = 300;

const MAX_BYTES = 35 * 1024 * 1024;

const columnMapKeySet = new Set(MODERN_JOB_COLUMN_KEYS as string[]);

const tabSchema = z
  .object({
    sheetName: z.string().min(1),
    bookYear: z.number().int().min(2020).max(2035),
...
export async function POST(req: Request) {
  ...
  type RunTabResult =
    | { ok: true; sheetName: string; bookYear: number; stats: JobImportTabResult }
    | { ok: false; sheetName: string; bookYear: number; error: string };
  ...
  for (const tab of config.tabs) {
    ...
    if (!sh) {
      results.push({
        ...
        sheetName: tab.sheetName,
        bookYear: tab.bookYear,
        error: "Missing sheet object",
      });
      continue;
    }
    const rows = sheetToRows(sh);
    try {
      const stats = await importJobBookTab(prisma, rows, {
        bookYear: tab.bookYear,
      ...
      results.push({ ok: true, sheetName: tab.sheetName, bookYear: tab.bookYear, stats });
    } catch (e) {
